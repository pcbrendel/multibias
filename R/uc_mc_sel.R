...
#'
#' \code{adjust_uc_mc_sel} returns the exposure-outcome odds ratio and confidence
...
#'
#' @examples 
#' adjust_uc_mc_sel(df_uc_mc_sel, exposure = "Xstar", outcome = "Y", 
...
adjust_uc_mc_sel <- function (data, exposure, outcome, confounders = NULL, px1_u0_parameters,
                              px0_u1_parameters, px1_u1_parameters, ps1_parameters, level = .95) {
  ...
  if (p4 != c + 3) {stop('Incorrect S1 parameter length')}
  ...
  if (is.null(confounders)) {
    ...
    final <- glm(Y ~ Xbar + Ubar, family = binomial(link = "logit"), weights = (pXU / pS), data = combined)
  ...
  if (c == 1) {
    ...
    final <- glm(Y ~ Xbar + C + Ubar, family = binomial(link = "logit"), weights = (pXU / pS), data = combined)
    est <- summary(final)$coef[2, 1]
    se <- summary(final)$coef[2, 2]
    alpha <- 1 - level
    return(list(exp(est), c(exp(est + se * qnorm(alpha / 2)), exp(est + se * qnorm(1 - alpha / 2)))))
    ...
    final <- glm(Y ~ Xbar + C1 + C2 + Ubar, family = binomial(link = "logit"),
                 weights = (pXU / pS), data = combined)
    est <- summary(final)$coef[2, 1]
    se <- summary(final)$coef[2, 2]
    alpha <- 1 - level
    return(list(exp(est), c(exp(est + se * qnorm(alpha / 2)), exp(est + se * qnorm(1 - alpha / 2)))))
    ...
    final <- glm(Y ~ Xbar + C1 + C2 + C3 + Ubar, family = binomial(link = "logit"),
                 weights = (pXU / pS), data = combined)
    est <- summary(final)$coef[2, 1]
    se <- summary(final)$coef[2, 2]
    alpha <- 1 - level
