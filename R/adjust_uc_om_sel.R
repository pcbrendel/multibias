...
# adjust_uc_om_sel_val() (data_validation input, method: imputation & weighting),
# adjust_uc_om_sel_coef_single() (bias_params input, method: imputation & weighting),
# adjust_uc_om_sel_coef_multinom() (bias_params input, method: weighting)
...
adjust_uc_om_sel_coef_single <- function(
  ...
  s_lp <- s1_0 + s1_x * df$X + s1_ystar * df$Ystar
  if (!is.null(confounders)) {
    C_matrix <- as.matrix(
      df[, paste0("C", seq_along(confounders)), drop = FALSE]
    )
...
adjust_uc_om_sel_coef_multinom <- function(
  ...
  # Calculate S predictions
  s_lp <- s1_0 + s1_x * combined$X + s1_ystar * combined$Ystar
  if (!is.null(confounders)) {
