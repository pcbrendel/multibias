...
An important confounder, smoking, was not included in our analysis! We'll specify this bias in our `data_observed()` object.
...
Let's demonstrate how we could use validation data with smoking information to adjust for the uncontrolled confounding. Feature `smoked_100cigs` corresponds to whether a participant reported having smoked 100 cigarettes throughout their life.
...
We now put this validation dataframe into `multibias_adjust()` along with a new `data_observed` object that identifies the uncontrolled confounding and exposure misclassification biases.
...
set.seed(1234)
multibias_adjust(df_obs2, df_val2)
```

The new bias-adjsted odds ratio is even closer to the null. The effect of alcohol is looking less meaningful when we bring these additional assumptions into consideration.
...
Now let's see if this differential study selection led to a bias of our alcohol risk effect. We'll add this validation data to `multibias_adjust` with the observed data that specifies the three biases.
...
set.seed(1234)
multibias_adjust(df_obs3, df_val3)
```

Any selection bias based on the sampling weights seems to be pretty minor; the bias-adjusted odds ratio of the effect of alcohol on mortality does not change much relative to the prior adjustment we made. Disagree with any of the bias assumptions made here? Multibias makes it very easy to re-iterate and test the sensitivity of the estimate to different assumptions.