...
vignette: >
  %\VignetteIndexEntry{Alcohol Risk}
...
nhanes_filtered <- nhanes |>
  ...
  mutate(smoked_100_cigs = case_when(
...
An important confounder was not included in our analysis, smoking! Let's demonstrate how we could use validation data with smoking information to adjust for the uncontrolled confounding. Feature `smoked_100_cigs` corresponds to whether a participant reported to having smoked 100 cigarettes throughout their life.

```{r}
df_temp1 <- df_filtered |>
...
We now put these two objects into the `multibias` function named `adjust_uc()` to adjust for uncontrolled confounding. This new bias-adjusted odds ratio () is closer to the null than that of our base model. This is not a surprise considering that extreme drinkers are more likely to be smokers, and smoking increases the risk of death. Part of the alcohol effect in our base model was really just the effect of smoking.
...
We now put these two objects into the `multibias` function named `adjust_uc_em()` to adjust for uncontrolled confounding and exposure misclassification. Observations
...
First, we sample with replacement based on each participant's survey weight to get a dataframe of selected participants. Then we'll make a dataframe of the participants who are not selected here.
...
Then make our ultimate dataframe here with an indicator for whether a given subject was or was not selected based on our sampling procedure.
...
Now let's see if this differential study selection led to a bias of our alcohol risk effect. We now put these two objects into the `multibias` function named `adjust_uc_em_sel()` to adjust for uncontrolled confounding, exposure misclassification, and selection bias. We observe an exposure odds ratio of _.
