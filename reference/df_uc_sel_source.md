...
`df_uc_sel` and can be used to obtain bias parameters for purposes of
validating the simultaneous multi-bias adjustment method with
`df_uc_sel`. With this source data, the fitted regression logit(P(Y=1))
= α₀ + α₁X + α₂C1 + α₃C2 + α₄C3 + α₅U shows that the true, unbiased
