...
create_causal_data <- function(
  ...
  if (!is.character(exposure) || length(exposure) != 1) {
    stop("Parameter 'exposure' must be a single string.")
  }
  if (!is.character(outcome) || length(outcome) != 1) {
    stop("Parameter 'outcome' must be a single string.")
  }
  if (!is.character(confounders)) {
    stop("Parameter 'confounders' must be a string or a vector of strings.")
  }
